...
begin
  ...
  //close
  nowTimeStr:=DateTimeToStr(Now);
  //save stevedore
  if DataModule_ado.getSeriary_number('wpc')>0 then
  begin
    CanClose:=False;
    Application.MessageBox('"辆序"有重复，请检查！','hint',MB_OK);
    Exit;
  end;
  DataModule_ado.addStevedore(DataModule_ado.ADODataSet_2task,nowTimeStr,taskVal);
  //save 2task
  DataModule_ado.save2task(tableNameStrB,nowTimeStr,
